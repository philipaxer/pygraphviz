...
def pkg_config():
    ...
    try:
        ...
        if output:
            library_path=output.strip()[2:].decode()
        output,err = \
                   S.Popen('pkg-config --cflags-only-I libcgraph',
                           shell=True, stdin=S.PIPE, stdout=S.PIPE,
                           close_fds=True).communicate()
        if output:
            include_path=output.strip()[2:].decode()
    except:
...
def dotneato_config():
    ...
    try:
        ...
        if output:
            include_path,library_path=output.split().decode()
            library_path=library_path.strip()[2:].decode()
            include_path=include_path.strip()[2:].decode()
        else:
